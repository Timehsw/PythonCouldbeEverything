...
class mysql2parquet():
    def __init__(self):
        ...
        self.database = MySQLdb.connect (host=self.hostname, user = self.properties.get('user'), passwd = self.properties.get('password'), db = self.dbname,charset="utf8")
        self.cursor = self.database.cursor()
        self.conf = SparkConf().setAppName(self.sparkname).setMaster("local[*]")
    ...
    def save_onetable(self,tablename):
        try:
            ...
            jdbcUrl = "jdbc:mysql://{0}:{1}/{2}?characterEncoding=utf8".format(self.hostname, self.jdbcPort, self.dbname)
            df = self.sqlContext.read.jdbc(url=jdbcUrl, table=tablename, properties=self.properties)
            df.show()
            # df.write.text("/home/hushiwei/output/"+tablename)
